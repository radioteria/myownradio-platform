...
run:
	docker run --rm --env-file "$(CURDIR)/.env" --name $(CONTAINER_ID) -p 6060:80 $(IMAGE_ID)

debug:
	docker run --rm -it --env-file "$(CURDIR)/.env" --name $(CONTAINER_ID) $(IMAGE_ID) bash

serve:
	docker run --rm -it --env-file "$(CURDIR)/.env" --name $(CONTAINER_ID) -p 6060:80 -v "$(CURDIR)":/usr/app/ $(IMAGE_ID)
